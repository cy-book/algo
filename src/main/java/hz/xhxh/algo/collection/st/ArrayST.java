...
public class ArrayST<K extends Comparable<K>, V> implements ComparableSymbolTable<K, V> {
    ...
    public ArrayST(){
        keys = (K[]) new Comparable[MIN_SIZE];
        values = (V[]) new Object[MIN_SIZE];
        N = 0;
    }
    ...
    public void put(K k , V v) {
        ...
        for(int j=N-1; j>i ;j--){
            values[j + 1] = values[j]; keys[j + 1] = keys[ j ];
        }
        values[i] = v;  keys[i] = k;
        N ++;
    ...
    public V delete(K k) {
        ...
        int i = rank(k);
        if(k.compareTo(keys[i]) != 0) return null;

        V v = values[i];
        for(int j=i; j<N; j++){
            values[j] = values[j+1];    keys[j] = keys[j+1];
        }
        values[N] = null;   keys[N] = null;
        N--;
